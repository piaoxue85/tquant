...
class DyStockBackTestingCtaEngine(object):
    ...
    def _processTicks(self, df):
        index = df.index.tolist()
        for lineIndex in index:
            ymd = lineIndex.strftime('%Y-%m-%d %H:%M:%S')[:10]
            h = lineIndex.strftime('%Y-%m-%d %H:%M:%S')[-8:][0:2]
            m = lineIndex.strftime('%Y-%m-%d %H:%M:%S')[-8:][3:5]
            s = lineIndex.strftime('%Y-%m-%d %H:%M:%S')[-8:][6:8]
            if h == '15' and (m != '00' or s != '00'):
                loc = datetime.strptime(ymd + ' 15:00:00', '%Y-%m-%d %H:%M:%S')
                df.loc[loc] = df.loc[lineIndex].values
                df.drop(lineIndex, axis=0, inplace=True)
    ...
    def _loadmBars(self, codes, m):
        ...
        for code in codes:
            ...
            #对于tick中15:00:01这种数据处理
            df = self._processTicks(df)
           # print(df)
            # 合成分钟Bar, 右闭合
            # 缺失的Bar设为NaN
            ...
            for i in range(0, 60*60*2 + 1, barM*60):
                ...
                if bars:
                    ...
                    if filteredBars:
                        ...
                        #     print('1', pos.availVolume, pos.totalVolume, pos.datetime)
